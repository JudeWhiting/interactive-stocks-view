...
def update_plot(graph_checklist: list, months_slider: int, checklist, sma_window, donchian_window, raw_data):
    ...
    fig.add_trace(go.Scatter(x=dates, y=[item["close"] for item in raw_data][-len(dates):], mode="lines", name="price"))
    if "Simple Moving Average" in checklist:
        sma = calculate_graph.simple_moving_average(raw_data, sma_window)
        fig.add_trace(go.Scatter(x=dates, y=sma[-len(dates):], mode="lines", name="sma"))
    if "Donchian Channel" in checklist:
        donchian = calculate_graph.donchian_channel(raw_data, donchian_window)
        fig.add_trace(go.Scatter(x=dates, y=donchian["max"][-len(dates):], mode="lines", name="donchian_max"))
        fig.add_trace(go.Scatter(x=dates, y=donchian["min"][-len(dates):], mode="lines", name="donchian_min"))
        fig.add_trace(go.Scatter(x=dates, y=donchian["avg"][-len(dates):], mode="lines", name="donchian_avg"))
